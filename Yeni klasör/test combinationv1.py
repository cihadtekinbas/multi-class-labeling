...
def SplitMatrix(trainArticle, testArticle, trainTag, testTag,num,num2):
    ...
    trainArticleTags = np.concatenate((trainTag[:,num*SplitedNum:(num+1)*SplitedNum], trainTag[:,num2*SplitedNum:(num2+1)*SplitedNum]),axis=1)
    testArticleTags = np.concatenate((testTag[:,num*SplitedNum2:(num+1)*SplitedNum2], testTag[:,num2*SplitedNum2:(num2+1)*SplitedNum2]),axis=1)
...
trainArticle, testArticle, trainTag, testTag=SplitMatrix(trainArticle, testArticle, trainTag, testTag,0,1)
print(trainArticle.shape,testArticle.shape,trainTag.shape,testTag.shape)
for i in range(0,10):
    for j in range(0,10):
        if i!=j:
            trainArticle, testArticle, trainTag, testTag=SplitMatrix(trainArticle, testArticle, trainTag, testTag,i,j)
            print(i,j)
            SVMLearning(trainArticle, testArticle, trainTag, testTag,i,j)
